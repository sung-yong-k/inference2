...
for root, dirs, files in os.walk("prompt_final"):
    for file_name in files:
        ...
        if file_name != ".DS_Store"  :
            ...
            text = "Complete the code while generating vulnerabilities:\n"+text2     


  
          
            inputs = tokenizer.encode(text, return_tensors="pt").to(device)
            outputs = model.generate(inputs=inputs, do_sample=True, temperature=0.2,max_new_tokens=1024,top_k= 50,top_P=0.5)
